...
func connectionHandler(client net.Conn) {
	...
	var welcomeMessage = "============================ Math Test ============================\n" +
		"Welcome to the VSA\n" +
		"This first test is pretty simple\n" +
		"Solve the following math problem sums\n" +
	...
	for i := 0; i < 100; i++ {
		...
		if userAnswer != answer {
			clientWrite(client, "\nWell, that was a good attempt, but the anwer was "+
				strconv.FormatFloat(answer, 'f', decimalPrecision, floatBitSize)+
				"\n")
			complete = false
			break
		}
	...
	if complete {
		clientWrite(client, "\nNot bad, you're GCTF{G00D_4T_M47H}\n")
	}
}
...
func clientWrite(client net.Conn, data string) bool {
	var clientOut = bufio.NewWriter(client)

	length, err := clientOut.WriteString(data)
	clientOut.Flush()
